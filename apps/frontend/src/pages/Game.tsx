...
export const CHECK_RECONNECT = "RECONNECT";
...
function PlayerCard({ name, color, active }: PlayerCardProps) {
    return (
        <div className={`flex items-center gap-3 rounded-xl px-4 py-2.5 border transition-all duration-200 ${active
            ? "bg-green-400/[0.04] border-green-400/35"
            : "bg-white/[0.025] border-white/[0.07]"
            }`}>
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border-2 ${color === "white"
...
export const Game = () => {
    ...
    useEffect(() => {
        ...
        const onOpen = () => {
            ...
            socket.send(JSON.stringify({ type: CHECK_RECONNECT }))
        }
        ...
        const onMsg = (e: MessageEvent) => {
            ...
            switch (msg.type) {
                ...
                case CHECK_RECONNECT:
                    chess.load(msg.payload.fen); // board state restore
                    setBoard(chess.board());
                    setStarted(true);
                    setPlayerColor(msg.payload.color);
                    break;
        ...
        if (socket.readyState === WebSocket.OPEN) {
            ...
            // if socket is not open then wait for it and when the socket will open in future then run the onOpen . 
            socket.addEventListener("open", onOpen);
        }

        socket.addEventListener("message", onMsg);
        return () => { socket.removeEventListener("open", onOpen); socket.removeEventListener("message", onMsg) };
    }, [socket, chess]);
